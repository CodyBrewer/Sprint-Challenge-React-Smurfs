...
- [ ] Explain the differences between `client-side routing` and `server-side routing`.
    ...
    Client side routing is done in the client browser. When a new page is request the client renders the route by accessing data available by the client. Server-side routing is done on the server. When a new page is requested the request is sent to the server which is then sends the client an entirley new page to send. 
